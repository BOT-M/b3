...
#ifndef _B3_MARKET_DATA_NEWS_5_CXX_H_
#define _B3_MARKET_DATA_NEWS_5_CXX_H_
...
public:
    ...
    {
        News_5 skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    #ifdef SBE_USE_SPAN
    SBE_NODISCARD std::span<const char> getLanguageCodeAsSpan() const SBE_NOEXCEPT
    {
    ...
    News_5 &putLanguageCode(std::span<const char, N> src) SBE_NOEXCEPT
    {
        static_assert(N <= 2, "array too large for putLanguageCode");
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    News_5 &putLanguageCode(T&& src)  SBE_NOEXCEPT requires
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    News_5 &putLanguageCode(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putLanguageCode(std::span<const char, N>(src));
    }
    #endif
